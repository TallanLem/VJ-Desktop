...
class DataRepository:
    ...
    def _fetch_gid_df_with_hash(self, gid: int) -> tuple[pd.DataFrame, str]:
        url = self._csv_url(GOOGLE_SHEET_ID, gid)
        resp = requests.get(url, timeout=30)
        resp.raise_for_status()
        csv_text = resp.text
        content_hash = hashlib.sha256(csv_text.encode("utf-8")).hexdigest()
        return pd.read_csv(io.StringIO(csv_text)), content_hash
    ...
    def sync_google_sheets(self) -> None:
        ...
        for group, gids in (("installation", INSTALLATION_GIDS), ("pump", PUMP_GIDS)):
            for gid in gids:
                source = f"{group}:{gid}"
                df, content_hash = self._fetch_gid_df_with_hash(gid)
                changed = self._sync_source_if_changed(df=df, source=source, content_hash=content_hash)
                payload["gids"].append(
                    {
                        "group": group,
                        "gid": gid,
    ...
    def _sync_source_if_changed(self, df: pd.DataFrame, source: str, content_hash: str) -> bool:
        cur = self.conn.cursor()
        row = cur.execute("SELECT content_hash FROM source_sync WHERE source_sheet=?", (source,)).fetchone()
        if row and row[0] == content_hash:
            return False

        self._replace_sheet(df=df, source=source)
        cur.execute(
            """
            INSERT INTO source_sync(source_sheet, content_hash, row_count, synced_at)
            VALUES(?, ?, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(source_sheet)
            DO UPDATE SET
                content_hash=excluded.content_hash,
                row_count=excluded.row_count,
                synced_at=CURRENT_TIMESTAMP
            """,
            (source, content_hash, int(len(df))),
        )
        self.conn.commit()
        return True

    @staticmethod
    def _pick_col(columns: list[str], preferred: str, fallback_tokens: list[str]) -> str | None:
        exact = next((c for c in columns if str(c).strip().lower() == preferred.lower()), None)
        if exact:
            return exact
        lowered = [(c, str(c).strip().lower()) for c in columns]
        return next((c for c, cl in lowered if any(token in cl for token in fallback_tokens)), None)

    def _replace_sheet(self, df: pd.DataFrame, source: str) -> None:
        columns = [str(c) for c in df.columns]
        article_col = self._pick_col(columns, preferred="Артикул", fallback_tokens=["артик", "article", "part"])
        name_col = self._pick_col(columns, preferred="Наименование", fallback_tokens=["наимен", "name", "model"])
        type_col = self._pick_col(columns, preferred="Тип", fallback_tokens=["тип", "type"])

        cur = self.conn.cursor()
        existing_ids = [r[0] for r in cur.execute("SELECT id FROM items WHERE source_sheet=?", (source,)).fetchall()]
        if existing_ids:
            placeholders = ",".join("?" for _ in existing_ids)
            cur.execute(f"DELETE FROM curves WHERE item_id IN ({placeholders})", existing_ids)
            cur.execute("DELETE FROM items WHERE source_sheet=?", (source,))
        ...
        for _, row in df.fillna("").iterrows():
            ...
            # индексированные поля поиска: fallback на raw_json заголовки
            if not article:
                article = raw.get("Артикул", "")
            if not name:
                name = raw.get("Наименование", "")
            cur.execute("UPDATE items SET article=?, name=? WHERE id=?", (article, name, item_id))
    ...
    def search_items(self, text: str, limit: int = 30) -> list[sqlite3.Row]:
        ...
        rows = self.conn.cursor().execute(
            "SELECT id, article, name, equipment_type FROM items ORDER BY name"
        ).fetchall()
        return [
            r
            for r in rows
            if needle in (r["article"] or "").casefold() or needle in (r["name"] or "").casefold()
        ][:limit]
